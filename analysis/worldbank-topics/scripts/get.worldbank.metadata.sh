...
SELECT DISTINCT ?identifier ?title ?topic
WHERE {
    ...
    GRAPH graph:meta {
        ...
        ?indicator property:topic <http://worldbank.270a.info/classification/topic/"${topic}"> .
        <http://worldbank.270a.info/classification/topic/"${topic}"> skos:prefLabel ?topic .
    }
}
