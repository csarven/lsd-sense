...
dataY <- read.csv(paste0(summaryPath, "correlation", ".", topic, ".", refPeriod, ".csv"), na.strings='', header=T)
...
write.csv(data, file=paste0(summaryPath, "similarity-correlation", ".", topic, ".", refPeriod, ".csv"), row.names=FALSE)
...
g <- ggplot(data, aes(data$similarity, data$correlation)) + xlab("Semantic similarity") + ylab("Correlation") + geom_point(alpha = 1/10) + ggtitle(paste0(refPeriod, " World Bank indicators with topic ", topic)) + theme_bw(base_size = 12, base_family = "")
...
g <- g + annotate("text", x=Inf, y=0, label=paste0("p-value: ", format(round(pValue, 5))), hjust=1.4, vjust=-2.75, size=4)
